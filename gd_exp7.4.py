...
def mc4(driftvec, rep, h, samples, t, b):
    ...
    totalsum = 0
    for i in range(rep):
        rbmpath = t * (-1 * RBM(driftvec, samples[i], h)[1][0:-1]) * h + b * (-1 * RBM(driftvec, samples[i], h)[1][0:-1]) **2 * h
        totalsum = totalsum + sum(rbmpath)
    expect = totalsum / rep
    return expect
    ...
    for id in range(nnn):
        ...
        for k in range(len(b_list)):
            ...
            if id == int(0) and k == int(0):
                ...
                data = data.append(df)
    data = data.reset_index(drop=True)
    data.to_csv('7.2plans_nobacktraking_init+0+0+0_coeb='+str(b)+'t='+str(t)+'sigma='+str(sigma)+'_ite100_b1e4_eta0.67_seed_' + str(seednow))
    return data


# best = driver2(1, 1000, -2, 1, 10)
...
best = driver2(1, 1000, -20000, 1, 10)
